...
class TestPatternMatchingS3:
    ...
    def test_match_pattern_s3_file(self, s3_uri):
        ...
        pattern = PatternMatcher(type=PatternType.LITERAL, pattern="oom-kill event")
    ...
    def test_context_extraction_s3(self, s3_uri):
        ...
        result = extractor.extract_context(s3_uri, line_number=22, context_lines=2)
...
class TestPatternMatchingSFTP:
    ...
    def test_match_pattern_sftp_file(self, sftp_uri):
        ...
        pattern = PatternMatcher(type=PatternType.LITERAL, pattern="Out of memory")
        matcher = PatternMatcherFactory.create_matcher(pattern)
    ...
    def test_context_extraction_sftp(self, sftp_uri):
        ...
        result = extractor.extract_context(sftp_uri, line_number=21, context_lines=3)
...
class TestPatternMatchingFTP:
    ...
    def test_match_pattern_ftp_file(self, ftp_uri):
        ...
        pattern = PatternMatcher(type=PatternType.LITERAL, pattern="Memory usage")
        matcher = PatternMatcherFactory.create_matcher(pattern)
    ...
    def test_context_extraction_ftp(self, ftp_uri):
        ...
        result = extractor.extract_context(ftp_uri, line_number=16, context_lines=1)
    ...
    def test_callable_matcher_ftp(self, ftp_uri):
        ...
        pattern = PatternMatcher(type=PatternType.CALLABLE, pattern="examples.custom_matchers:detect_slurm_oom_kill")
        matcher = PatternMatcherFactory.create_matcher(pattern)
...
class TestErrorMatchBuilding:
        ...
        with FileStream.open_file(local_oom_log) as stream:
            for line_num, line_text in stream.read_lines():
                if matcher.match(line_text):
                    # Extract context
                    extractor = ContextExtractor()
                    context = extractor.extract_context(local_oom_log, line_num, error_def.context_lines)
    ...
    def test_build_match_from_s3_file(self, s3_uri):
        ...
        error_def = ErrorDefinition(
            ...
            pattern=PatternMatcher(type=PatternType.REGEX, pattern=r"Exceeded.*memory limit", flags=["IGNORECASE"]),
            files=[s3_uri],
        ...
        with FileStream.open_file(s3_uri) as stream:
            for line_num, line_text in stream.read_lines():
                if matcher.match(line_text):
                    context = extractor.extract_context(s3_uri, line_num, error_def.context_lines)
...
class TestMultiProtocolSupport:
    ...
    def test_same_pattern_all_protocols(self, local_oom_log, s3_uri, sftp_uri, ftp_uri):
        ...
        pattern = PatternMatcher(type=PatternType.REGEX, pattern=r"SLURM Job \d+", flags=["IGNORECASE"])
        matcher = PatternMatcherFactory.create_matcher(pattern)
